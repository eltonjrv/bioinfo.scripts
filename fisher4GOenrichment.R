...
for (i in c(i:n)) {onList_onGO = x[i,2]; onList_notGO = glist - x[i,2]; onGO_notList = x[i,3] - x[i,2]; notList_notGO = total_genes_wGO - onList_onGO - onList_notGO - onGO_notList; y <- matrix(c(onList_onGO, onList_notGO, onGO_notList, notList_notGO), 2, 2, byrow=T); print(fisher.test(y)$p.value);}
