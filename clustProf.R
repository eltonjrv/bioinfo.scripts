...
y = enrichGO(x[,8], OrgDb=org.Hs.eg.db, ont="ALL", keyType="SYMBOL")
...
r <- enrichPathway(gene=z[,2], pvalueCutoff = 0.05, readable=TRUE)
outREACT = gsub(".xls", "-enrichREACT.tsv", args[1])
...
k = enrichKEGG(z[,2], organism="hsa", keyType="ncbi-geneid")
outKEGG = gsub(".xls", "-enrichKEGG.tsv", args[1])
